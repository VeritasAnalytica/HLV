...
export class TwitterPostClient {
    ...
    async generateNewTweet() {
        ...
        try {
            ...
            // Calculate available space for content considering URL and space between
            ...
            // Truncate the content to fit with the URL
            if (maxContentLength > 0) {
                ...
                cleanedContent = `${cleanedContent} ${boostLotteryUrl}`;
            }
    ...
    private async handleTextOnlyReply(
        ...
        try {
            ...
            if (tweet.photos?.length > 0) {
                ...
                for (const photo of tweet.photos) {
                    const description = await this.runtime
                        ...
                        .describeImage(photo.url);
                    imageDescriptions.push(description.description);
                }
            }
            ...
            const maxTweetLength = this.client.twitterConfig.MAX_TWEET_LENGTH;
            const urlLength = boostLotteryUrl.length;
            const spaceBetween = 1; // one space between text and URL
            const maxContentLength = maxTweetLength - urlLength - spaceBetween;

            // Clean up and truncate reply text if necessary
            replyText = removeQuotes(fixNewLines(replyText)).trim();
            if (maxContentLength > 0) {
                replyText = truncateToCompleteSentence(
                    replyText,
                    maxContentLength
                );
            }
            // Append the URL ensuring a space separates the text and URL
            replyText = `${replyText} ${boostLotteryUrl}`;
            // ---- End URL Concatenation Logic ----
            ...
            // --- NEW: Approval Check ---
            if (this.approvalRequired) {
                elizaLogger.log(`Sending reply for approval: ${replyText}`);
                const discordMessageId = await this.sendForApproval(
                    replyText,
                    stringToUuid(
                        tweet.conversationId + "-" + this.runtime.agentId
                    ),
                    replyText,
                    tweet.id, // Pass the tweet ID being replied to
                    tweet.username // Pass the original tweet's username
                );
                if (discordMessageId) {
                    executedActions.push("reply (pending approval)");
    ...
    private async sendForApproval(
        ...
        newTweetContent: string,
        inReplyToTweetId?: string,
        originalUsername?: string // Add this new parameter
    ): Promise<string | null> {
        try {
            const title = inReplyToTweetId
                ? "New Tweet Reply Pending Approval"
                : "New Tweet Pending Approval";

            // Construct the reply URL if we have both the tweet ID and username
            const replyToField =
                inReplyToTweetId && originalUsername
                    ? `https://twitter.com/${originalUsername}/status/${inReplyToTweetId}`
                    : inReplyToTweetId
                      ? `Tweet ID: ${inReplyToTweetId}`
                      : "New Tweet";
        ...
        for (const pendingTweet of pendingTweets) {
            ...
            if (approvalStatus === "APPROVED") {
                ...
                try {
                    let result;
                    if (
                        pendingTweet.cleanedContent.length >
                        DEFAULT_MAX_TWEET_LENGTH
                    ) {
                        result = await this.handleNoteTweet(
                            this.client,
                            pendingTweet.cleanedContent,
                            pendingTweet.inReplyToTweetId // Pass reply target
                        );
                    } else {
                        result = await this.sendStandardTweet(
                            this.client,
